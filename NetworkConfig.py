...
class Network:
    ...
    def StartThreadingOnAll(self):
        ...
        for i in self.nodes:
         ...
         threads.append(t)
         t.start()
    def ReStartThreadingOnOne(self,port):


         i = self.getNodeByPort(port)
         t = threading.Thread(target=i.zmqRead, args=())
         t.daemon = True

         t.start()

    def getSizeOfNode(self,node):
        return sys.getsizeof(self.getNodeByPort(node))
    def getSizeOfTangle(self,node):
        return sys.getsizeof(self.getNodeByPort(node).getTangle())
    def getSizeOfAllTangles(self):
        print("[sizes :{")
        count = 0
        for i in self.nodes:
            print("Size of tangle "+ str(count)+" : " + str(sys.getsizeof(i.getTangle())) + " bytes")
            count = count + 1
        print("        }\n ]")
    def getSizeOfAllNodes(self):
        print("[sizes :{")
        count = 0
        for i in self.nodes:
            print("Size of Node "+ str(count)+" : " + str(sys.getsizeof(i)) + " bytes")
            count = count + 1
        print("        }\n ]")
    def EnablePacketDrop(self):
        self.droppackets = 1

    def DisablePacketDrop(self):
        self.droppackets = 0
...
edges = [(1,4),(1,3),(2,4),(2,3),(3,5),(3,6),(4,7),(4,8),(6,7),(3,4),(5,6)]
